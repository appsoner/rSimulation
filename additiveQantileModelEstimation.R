################################################################
####Nonparameterric estimatiom of the addtive quantile model####
################################################################
#----------------------------------------------------------------
#Description:   this is a demo estimating addtive quantile model,
#               the program is derived from "Nonparameteric Estimation
#               of an Additive Quantile Regression Model" on JASA.
#               The code inspired by Joel L. Horowitz, the author of
#               foregoing paper on JASA, can be wrapped to be a function.
#Model:         y = exp(x)+sin(2*pi*t)+z+random_term
#----------------------------------------------------------------
#Author:        Guosheng Xu
...
plot(tmp2, tmp3, ylim = c(min(tlb1),max(tub1)), type="l", col = "blue", xlab = expression(paste(X)), ylab = expression("exp()"), cex = 0.7, cex.axis = 0.85, lwd = 2)
lines(tmp2, tlb1,  lty=2, col = "red")
lines(tmp2, tub1,  lty=2, col = "red")


plot(tmp5, tmp6, ylim = c(min(tlb2),max(tub2)), type="l", col = "blue", xlab = expression(t), ylab = expression("sin()"), cex = 0.7, cex.axis = 0.85, lwd = 2)
lines(tmp5, tlb2,  lty=2, col = "red")
lines(tmp5, tub2,  lty=2, col = "red")

plot(tmp8, tmp9, ylim = c(min(tlb3),max(tub3)), type="l", col = "blue", xlab = " Age ", ylab = " ", cex = 0.7, cex.axis = 0.85, lwd = 2)
lines(tmp8, tlb3,  lty=2, col = "red")
lines(tmp8, tub3,  lty=2, col = "red")
...
plot(tmp2, tmp3, ylim = c(min(tmp3),max(tmp3)), type="p", col = rgb(0.2, 0.1, 0.5, 0.5), xlab = " TOPTEN ", ylab = " ", cex = 0.7, cex.axis = 0.85, pch = 19)
abline(h=0, col = "gray", lty = 2, lwd = 2)


plot(tmp5, tmp6, ylim = c(min(tmp6),max(tmp6)), type="p", col = rgb(0.2, 0.1, 0.5, 0.5), xlab = " Log(Assets) ", ylab = " ", cex = 0.7, cex.axis = 0.85, pch = 19)
abline(h=0,col = "gray", lty = 2, lwd = 2)


plot(tmp8, tmp9, ylim = c(min(tmp9),max(tmp9)), type="p", col = rgb(0.2, 0.1, 0.5, 0.5), xlab = " Age ", ylab = " ", cex = 0.7, cex.axis = 0.85, pch = 19)
abline(h=0, col = "gray", lty = 2, lwd = 2)
